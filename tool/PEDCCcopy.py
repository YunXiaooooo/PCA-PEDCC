...
PEDCC_g_T = os.path.join(PEDCC_root,str(class_num)+"_"+str(latent_variable_dim)+"T_s.pkl")
PEDCC_ui=os.path.join(PEDCC_root,str(class_num)+"_"+str(latent_variable_dim)+"_s.pkl")
#T_s 是旋转以前，_s是旋转以后，一般用_s即可
...
f=open('./center_pedcc/GausiNew/Guasi_matrix.txt', 'a+')
gausi = str(np.array(u))
f.write(gausi + '\n')
f.close()
print(np.array(u)[:, -9:])
...
ggg_T = open(PEDCC_g_T, 'wb')
gmap = {}
for i in range(len(g_T)):
    gmap[i]=torch.from_numpy(np.array([g_T[i]]))
pickle.dump(gmap,ggg_T)
ggg_T.close()
